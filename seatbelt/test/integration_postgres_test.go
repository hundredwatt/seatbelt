...
import (
	...
	"seatbelt/pkg/postgres2"  // Local package import
...
var table_definition = &seatbelt.TableDefinition{
	TableName:      "public.data_proof_test",
	PrimaryKeyName: "id",
...
type TestingRowMapperAndHasher struct {
	postgres2.PostgresDefaultSourceHasher
	seatbelt.NoopTargetHasher
}
...
func TestPostgres2_Scan(t *testing.T) {
	pool := setupTestDB(context.Background(), t)
	defer pool.Close()

	source := postgres2.NewPostgresSource(pool)
	...
	assert.Equal(t, scan.RowCount(), int64(25))
	id25_line, err := testutil.FindLineWithPrefix(scan.File, "25,")
	assert.NoError(t, err)
	assert.Equal(t, id25_line, "25,3627417695111965652")
}

func TestPostgres2_ExtractScan(t *testing.T) {
	t.Skip("Skipping extract scan test")
	pool := setupTestDB(context.Background(), t)
	defer pool.Close()
	...
	assert.Equal(t, extract_scan.RowCount(), int64(25))

}

func TestPostgres2_ConsumeChangeStream(t *testing.T) {
	...
	assert.Equal(t, source_changes.RowCount(), int64(25))
	id25_line, err := testutil.FindLineWithPrefix(source_changes.File, "25,")
	assert.NoError(t, err)
	assert.Equal(t, id25_line, "25,3627417695111965652,3627417695111965652")

	// testutil.Preview(source_changes.File, 30)
}
