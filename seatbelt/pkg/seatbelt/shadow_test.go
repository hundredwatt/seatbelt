...
func TestInitialLoad(t *testing.T) {
	...
	// Initial load only needs source extract scan and target scan
	data_files := &seatbelt.DataFileSet{
		TargetScan:        &seatbelt.DataFile{File: target_scan_file},
		SourceExtractScan: &seatbelt.DataFile{File: source_extract_scan_file},
	}

	shadow_file, err := os.CreateTemp("", "shadow_initial.csv")
	if err != nil {
		t.Fatalf("Failed to create shadow file: %v", err)
	}
	cfg := &seatbelt.Config{
